...
import {TicketValidation} from '../../dtos/ticket-validation';
...
export class InvoicePdfPrintComponent implements OnInit {
  ...
  private companyName: String = 'INSO AustriaTicket GmbH';
  private street: String = 'Wiedner Hauptstraße 76/2/2';
  private city: String = '1040 Wien';
  private telefon: String = '+43 1 5872197';
  private uid: String = 'ATU12345678';
  private bic: String = 'BKAUATWW';
  private iban: String = 'AT121234501234567891';
  private qrCode: any;
  private qrLoaded: any[] = [];
  private qrUrlLoaded: any[] = [];
  ...
  createPDF() {
    ...
    this.getQRCode();
    Promise.all(this.qrLoaded).then(function (qrLoaded) {
      Promise.all(this.qrUrlLoaded).then(function (qrUrlLoaded) {
        ...
        doc.addImage(this.qrCode, 'PNG', 90, yOffset, 30, 30);
  ...
  getQRCode() {
    const promise = this.invoicePdfService.getQR(this.invoice.id).toPromise();
    promise.then(function (result) {
      this.appendQRFromBlob(result);
    }.bind(this));
    this.qrLoaded.push(promise);
  }

  appendQRFromBlob(qr: Blob) {
